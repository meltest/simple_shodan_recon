...
echo "============================"
#zcat recon_$TIMESTAMP/*.json.gz | sed -e 's/\\u[0-9A-Fa-f]\{4\}//g' | jq -cr '[ .ip_str, .port, if .os then .os else "-" end, if .domains != "" then .domains|sort|join("|") else "-" end, if .product then .product else "-" end, if .version then .version else "-" end, if .vulns then .vulns|keys|sort|join("|") else "-" end, .timestamp ] | @csv' | sed '1s/^/"ip", "port", "os", "domains", "product", "version", "vulns", "timestamp"\n/' | tee recon_$TIMESTAMP/simple_shodan_recon_$TIMESTAMP.csv
zcat recon_$TIMESTAMP/*.json.gz | sed -e 's/\\u[0-9A-Fa-f]\{4\}//g' | jq -cr '[ .ip_str, .port, if .os then .os else "-" end, if .domains | length > 0 then .domains|sort|join("|") else "-" end, if .product then .product else "-" end, if .version then .version else "-" end, if .vulns then .vulns|keys|sort|join("|") else "-" end, .timestamp ] | @csv' | sed '1s/^/"ip", "port", "os", "domains", "product", "version", "vulns", "timestamp"\n/' | tee recon_$TIMESTAMP/simple_shodan_recon_$TIMESTAMP.csv
